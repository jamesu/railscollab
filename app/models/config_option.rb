...
class ConfigOption < ApplicationRecord
  ...
  def self.reload_all
    if Rails.configuration.railscollab.server == :fastcgi
    ...
    unless [:passenger, :fastcgi].include? Rails.configuration.railscollab.server or Rails.configuration.railscollab.dont_reload_config
