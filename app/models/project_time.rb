...
class ProjectTime < ActiveRecord::Base
  ...
  def open_task_id
    return '0' if self.project_task.nil?
  ...
  def is_today?
    self.done_date.to_date == Date.today
  end

  def is_yesterday?
    self.done_date.to_date == Date.yesterday
  end
  ...
  def self.find_by_task_list(params, time_conds, time_order=nil)
    ...
    ProjectTaskList.all(:params).each do |list|
      tasks = []
      list.project_tasks.each do |task|
        ...
        if (!total.nil? and total > 0)
          ...
          tasks << {:task => task, :hours => total, :list => ProjectTime.all(:conditions => extra_conditions, :order => time_order)}
        end
      end
  ...
  def self.find_grouped(group_field, params)
    grouped_fields = {}
    found_times = ProjectTime.all(params)
  ...
  def can_be_edited_by(user)
    ...
    (user.is_admin or created_by.id == user.id) and project.is_active?
  end
  ...
  def can_be_seen_by(user)
    return false unless project.has_member(user)
    return true  if user.has_permission(project, :can_manage_time)
