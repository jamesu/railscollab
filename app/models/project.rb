...
class Project < ActiveRecord::Base
  ...
  has_many :project_times, :dependent => :destroy do
    def public(reload=false)
      ...
      @public_project_times ||= all(:conditions => ['is_private = ?', false])
    end
  end
  ...
  has_many :project_milestones, :dependent => :destroy do
    def public(reload=false)
      ...
      @public_project_milestones ||= all(:conditions => ['is_private = ?', false])
    end

    def open(include_private=true, reload=false)
      # Grab open milestones only
      ProjectMilestone.priv_scope(include_private) do
        all(:conditions => 'project_milestones.completed_on IS NULL', :order => 'project_milestones.due_date ASC')
      end
    end

    def late(include_private=true, reload=false)
      ProjectMilestone.priv_scope(include_private) do
        all(:conditions => ['due_date < ? AND completed_on IS NULL', Date.today])
      end
    end

    def todays(include_private=true, reload=false)
      ProjectMilestone.priv_scope(include_private) do
        all(:conditions => ['completed_on IS NULL AND (due_date >= ? AND due_date < ?)', Date.today, Date.today+1])
      end
    end

    def upcoming(include_private=true, reload=false)
      ProjectMilestone.priv_scope(include_private) do
        all(:conditions => ['completed_on IS NULL AND due_date >= ?', Date.today+1])
      end
    end

    def completed(include_private=true, reload=false)
      ProjectMilestone.priv_scope(include_private) do
        all(:conditions => 'completed_on IS NOT NULL')
      end
    end
  end
  ...
  has_many :project_task_lists, :order => "#{self.connection.quote_column_name 'order'} DESC", :dependent => :destroy do
    def public(reload=false)
      ...
      @public_project_task_lists ||= all(:conditions => ['is_private = ?', false])
    end

    def open(include_private = true, reload=false)
      ProjectTaskList.priv_scope(include_private) do
        # Grab open task lists only
        all(:conditions => 'project_task_lists.completed_on IS NULL')
      end
    end
    ...
    def completed(include_private = true, reload=false)
      ProjectTaskList.priv_scope(include_private) do
        ...
        all(:conditions => 'project_task_lists.completed_on IS NOT NULL')
      end
    end
  end
  ...
  has_many :project_files, :dependent => :destroy do
    def important(include_private = true, reload=false)
      ProjectFile.priv_scope(include_private) do
        all(:conditions => ['is_important = ?', true])
      end
    end
  end
  ...
  has_many :application_logs, :order => 'created_on DESC, id DESC', :dependent => :destroy do
    def public(reload=false)
      ...
      @public_application_logs ||= all(:conditions => ['is_private = ?', false])
    end
  end
  ...
  def tasks_by_user(user, completed=false)
    ProjectTask.all(:conditions => ['((assigned_to_company_id = ? OR assigned_to_user_id = ?) OR (assigned_to_company_id = 0 OR assigned_to_user_id = 0)) AND completed_on ? NULL', user.company_id, user.id, completed ? 'IS NOT' : 'IS'])
  end
  ...
  def milestones_by_user(user, completed=false)
    ProjectMilestone.all(:conditions => ['project_id = ? AND ((assigned_to_company_id = ? OR assigned_to_user_id = ?) OR (assigned_to_company_id = 0 OR assigned_to_user_id = 0)) AND completed_on ? NULL', self.id, user.company_id, user.id, completed ? 'IS NOT' : 'IS'])
  end

  def has_member(user)
    ProjectUser.first(:conditions => ['project_id = ? AND user_id = ?', self.id, user.id], :select => 'user_id')
  end
  ...
  def can_be_seen_by(user)
    self.has_member(user) or (user.member_of_owner? and user.is_admin)
  end
