...
class FormController < ApplicationController
  ...
  def index
    unless (@logged_user.is_admin and @logged_user.member_of_owner?)
  ...
  def submit
    unless @form.can_be_submitted_by(@logged_user)
      error_status(true, :insufficient_permissions)
      redirect_back_or_default :controller => 'form'
      return
    end
  ...
  def add
    ...
    unless ProjectForm.can_be_created_by(@logged_user, @active_project)
      error_status(true, :insufficient_permissions)
  ...
  def edit
    unless @form.can_be_edited_by(@logged_user)
      error_status(true, :insufficient_permissions)
      redirect_back_or_default :controller => 'form'
      return
    end
  ...
  def delete
    unless @form.can_be_deleted_by(@logged_user)
      error_status(true, :insufficient_permissions)
      redirect_back_or_default :controller => 'form'
      return
    end
