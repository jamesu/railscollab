...
xml.rss "version" => "2.0", "xmlns:dc" => "http://purl.org/dc/elements/1.1/" do
  ...
  xml.channel do 
    xml.title I18n.t('recent_activities_feed')
    xml.link @activity_url
    xml.description I18n.t('recent_activities_feed_info')

    xml.language 'en-us'
    @activity_log.each do |activity|
      xml.item do
        item_url = activity.rel_object ? activity.rel_object.object_url(Rails.configuration.railscollab.site_url) : root_url
